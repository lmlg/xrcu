...
As such, lock freedom isn't feasible as is in languages like C and C++. We need
...
that allows us multiple threads to I<read> from memory, while postponing
...
safely nested without problem. As long as we are in a critical sections,
...
Adds the object F to the current thread's list of pending finalizable objects.
