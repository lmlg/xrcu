...
Hash tables are associative that maps unique keys to values. Ordering is
unspecified for both keys and values. In addition to the key and value types,
hash tables are instantiated with a Hashing type and an equality types;
callables that compute a I<hash value> for a given key, and one that tests for
equality, given 2 keys, respectively.
...
key, value, hashing and equality types, in that order. The C<Hash> type has to
operate on keys and return unsigned integers, whereas the C<Equal> type has to
...
C<lf>, using the equality predicate C<e> and the hasher C<h>. If the load
factor must be in the range [0.4, 0.9]; if it's not, then it will silently be
set to the default of 0.85.
...
other parameters work as with the previous constructor.
...
Updates the value associated to C<key> by calling the C<f> with it and the rest
of the arguments. In other words, if C<val> is the value associated to key,
calls C<f (val, args...)> and updates the value with the result. If no value
was present for that key, the function is called with a default constructed
value instead (as if by calling C<Val ()>).
...
be any updates made. The function can also receive and return a freshly made
object, and C<update> will work the same. The function may be called more than
once if the atomic updates fail.

Returns true if C<key> was present before the call.
