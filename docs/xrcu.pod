...
the arguments. In other words, if C<val> is the value associated to key, calls
C<f (val, args...)> and updates the value with the result. If no value was
present for that key, the function is called with a default constructed value
