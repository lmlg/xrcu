...
a special I<sentinel> bit, that is temporarily set as the head node when a
swap is undergoing. During a swap, C<push> and C<pop> cannot proceed, since
they check against that the special bit is not set before modifying the stack.
...
forced to reallocate. The read and write cursors and indices that indicate
